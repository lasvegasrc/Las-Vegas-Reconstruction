...
void HashGrid<VertexT, BoxT>::addLatticePoint(int index_x, int index_y, int index_z, float distance)
{
    ...
    // Extrude in all directions (26 blocks around the current one), because we iterate over all points
    // in the point cloud and therefore cant predict a direction from which we came.
